...
lineaccesstoken = '0Mw6sn1e4oykmPpA84d8Oz4Dglx132qQdTj6N9TXHW7DVE/b9v7s7TtRLxk0rWVPJDgCA8wXk9l8anJmmYpxrTQlmwXhvhagEAcjZyvBchk2afhhLrKPUYrGUyeomyGaGLFljG2dRRGd1icvXE0bVQdB04t89/1O/w1cDnyilFU='
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="สวัสดีค้าบ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "สบายดีมั้ย") :
            replyObj = TextSendMessage(text="สบายดีค้าบผม")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "กินข้าวยัง") :
            replyObj = TextSendMessage(text="ยังเลยคั้บ;-;")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'dialogflow.cloud.google.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/9ece3673-b47a-4c29-bea3-77ccefc8791e"
